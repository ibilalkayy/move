...
fn credentials(
    conn: &Connection,
) -> Result<(String, String, String, String, u64), rusqlite::Error> {
    let mut stmt = conn.prepare(
        "select private_key, private_key_nonce, alchemy_url, alchemy_url_nonce, chain_id from blockchain"
    ).expect("❌ Failed to find the blockchain credentials");
...
pub fn give_data(
    ...
    let (private_key, private_key_nonce, alchemy_url, alchemy_url_nonce, chain_id) =
        credentials(conn)?;
